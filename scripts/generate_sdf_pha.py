...
from repeat_main import pha_convert


def batch_generate_mols_from_csv_pha(
    input_csv: str,
    key_column: str,
    left_cap: str,
    right_cap: str,
    ...
    """
    从 CSV 中批量生成 .sdf 文件和 SMILES 表格（用于下游模型）
    """
    ...
    for idx, row in df.iterrows():
        ...
        mol = pha_convert(left, right, repeat, unit_num)
...
if __name__ == "__main__":
    batch_generate_mols_from_csv_pha(
        input_csv="E:\\ohter\\trans\\plastic\\already.xlsx",
        key_column="plastic",
        left_cap="left",
        right_cap="right",
        repeat_column="repeat",
        unit_num=10,
        output_dir="mols_for_unimol_10_sdf_pha"
    )